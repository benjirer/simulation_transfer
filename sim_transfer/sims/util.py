...
def delay_and_stack_spot_actions(
    ...
    if action_stacking:
        ...
        print(
            "[data_provider] Using action stacking with max action delay (we can't split action delay if we use action stacking)",
            action_delay,
        )
    ...
    # only action delay
        ...
        u_base, u_ee = u[:, :3], u[:, 3:]
        print(
            "[data_provider] Using action delay for base and ee actions with delays",
            action_delay_base,
            action_delay_ee,
        )
